...
import { UploadCloud, Shield, Activity, Terminal } from 'lucide-react';
import AudioVisualizer from './AudioVisualizer'; // Ensure this path matches where you saved the component
...
function App() {
  ...
  const [visualData, setVisualData] = useState(null) // NEW: State to hold chart data
  ...
  const handleFileSelect = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      setVerdict(null)
      setVisualData(null) // Clear previous visuals on new file
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      // Pass the current slider thresholds to the backend
      formData.append("minCentroid", minCentroid)
      formData.append("minZcr", minZcr)
      formData.append("maxMfccVar", maxMfccVar)

      // ⚠️ CRITICAL: Replace YOUR_CODESPACE_URL_HERE with your actual forwarded URL
      const response = await fetch('https://humble-computing-machine-g4wq5x9pjp5jf97jx-8000.app.github.dev/scan', { 
        method: 'POST',
        body: formData 
      })
      ...
      if (data.status === "success") {
        setStatus(`Verdict: ${data.verdict} (Confidence: ${data.confidence})`)
        setVerdict(data.verdict.toLowerCase()) // Set to 'safe' or 'threat'
        setVisualData(data.visuals) // Inject data into charts
        
        // Dynamically Generate History Log Entry
        ...
        const newLogEntry = {
          ...
          centroid: data.metrics.centroid.toFixed(0), 
          zcr: data.metrics.zcr.toFixed(3), 
          status: data.verdict
        };

        setScanHistory(prevHistory => [newLogEntry, ...prevHistory]);
      } else {
        setStatus(`System Error: ${data.message}`)
        setVerdict('error')
      }

    } catch (error) {
      setStatus("System Error: Connection refused. Check Codespace URL mapping.")
      setVerdict('error')
    } finally {
      setIsScanning(false)
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 font-sans antialiased flex flex-col justify-between selection:bg-sky-500/30 overflow-y-auto">
      
      {/* Top Professional Navigation Bar */}
      <header className="border-b border-zinc-900 bg-zinc-900/20 backdrop-blur-md px-8 py-4 flex items-center justify-between sticky top-0 z-50">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-6 py-8">
        ...
        {/* BOTTOM SECTION: Acoustic Maps (Conditionally Rendered) */}
        {visualData && (
          <div className="w-full animate-fade-in transition-all duration-500">
            <AudioVisualizer visuals={visualData} />
          </div>
        )}
